...
import { apiService } from '../../services/apiService'; // Importación del servicio

export function AdminDashboard({ onLogout, userData }) {
  const [activeTab, setActiveTab] = useState('dashboard');
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  
  // Validamos contra 'SUPER_ADMIN' (tu base de datos) o 'superadmin'
  const isSuperAdmin = userData?.role === 'SUPER_ADMIN' || userData?.role === 'superadmin';

  // Estados reales manejados por la API
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Si es SuperAdmin, cargar usuarios
        if (isSuperAdmin) {
  ...
  const getStatusColor = (status) => {
    const colors = { completed: '#28a745', 'in-progress': '#ffc107', upcoming: '#17a2b8', pending: '#17a2b8' };
  ...
  const handleApproveUser = async (userId) => {
    try {
      ...
      // Mover de pendientes a activos en la UI
      const approvedUser = pendingUsers.find(u => u.id === userId);
      setPendingUsers(pendingUsers.filter((u) => u.id !== userId));
      if (approvedUser) {
        setActiveUsers([...activeUsers, { ...approvedUser, status: 'ACTIVE' }]);
      }
    } catch (error) {
  ...
  const handleRejectUser  = async (userId) => {
    try {
      ...
      alert("Hubo un error al rechazar al usuario.");
    }
  };
        ...
        <div className="p-4 lg:p-6">
          {isLoading ? (
             <div className="flex justify-center items-center h-64">
               <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#f18517]"></div>
             </div>
          ) : (
            <>
              {/* Dashboard */}
                      ...
                      <div key={project.id} className="flex flex-col sm:flex-row sm:items-start gap-4 p-4 border rounded-lg hover:shadow-md transition-shadow" style={{ borderColor: '#e0e0e0' }}>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm" style={{ color: '#808080' }}>{getStatusLabel(project.status)} - {project.progress || 0}%</span>
                          </div>
                        </div>
                      </div>
                    ))}
                        ...
                        <div className="space-y-4">
                          {pendingUsers.map((user) => (
                            <div key={user.id} className="flex flex-col p-4 border rounded-lg gap-4" style={{ borderColor: '#e0e0e0' }}>
                              <div className="flex-1">
                                <h3 className="text-base lg:text-lg mb-1" style={{ color: '#1d1d1b', fontFamily: 'Montserrat, sans-serif', fontWeight: 500 }}>{user.fullName || user.name}</h3>
                                ...
                                <p className="text-xs" style={{ color: '#808080' }}>Solicitud: {user.requestDate || 'Desconocida'}</p>
                              </div>
                        ...
                        <tbody>
                          {activeUsers.map((user) => (
                            <tr key={user.id} className="border-b" style={{ borderColor: '#e0e0e0' }}>
                              <td className="p-3 lg:p-4 text-sm" style={{ color: '#1d1d1b' }}>{user.username}</td>
                              <td className="p-3 lg:p-4 text-sm" style={{ color: '#808080' }}>{user.email}</td>
                              <td className="p-3 lg:p-4">
                                <span className="px-2 py-1 rounded-full text-xs" style={{ backgroundColor: user.role === 'SUPER_ADMIN' ? '#f18517' : '#6c757d', color: 'white' }}>
                                  {user.role}
                                </span>
                              </td>
                              <td className="p-3 lg:p-4">
                          ...
                          {activeUsers.length === 0 && (
                             <tr>
                               <td colSpan="4" className="p-4 text-center text-gray-500">No hay usuarios activos cargados.</td>
                             </tr>
                          )}
                        </tbody>
                      </table>
